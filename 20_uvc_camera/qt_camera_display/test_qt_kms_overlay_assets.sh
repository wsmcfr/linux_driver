...
require_grep "deviceHealth.locationStatusText" "qml/Main.qml"
require_grep "deviceHealth.locationDisplayText" "qml/Main.qml"
require_grep "deviceHealth.locationShortText" "qml/Main.qml"
require_grep "位置" "qml/Main.qml"
require_grep "定位" "qml/Main.qml"
...
require_grep "40000" "main.cpp"
require_grep "m_locationBootProbeDone" "main.cpp"
require_grep "startLocationProbe\\(true\\)" "main.cpp"
require_grep "startLocationProbe\\(false\\)" "main.cpp"
require_grep 'QStringLiteral\("status"\)' "main.cpp"
require_grep 'QStringLiteral\("once"\)' "main.cpp"
require_grep "startLocationProbe" "main.cpp"
require_grep "handleLocationProcessFinished" "main.cpp"
require_grep "handleLocationProbeTimeout" "main.cpp"
require_grep "setLocationStatus" "main.cpp"
require_grep "parseKeyValueOutput" "main.cpp"
if grep -Eq 'LOCATION_ACTIVE_PROBE_INTERVAL_MS|m_locationActiveProbeElapsed' "$SCRIPT_DIR/main.cpp"; then
    fail "main.cpp 不能再按 5 分钟重复主动定位；开机后只允许 once 一次，后续只能读取 status 缓存"
fi
...
require_grep "timestampedLogFilePath" "main.cpp"
require_grep "sanitizeLogFileToken" "main.cpp"
require_grep "writeTextFileWithFsync" "main.cpp"
...
if grep -Eq "DEFAULT_ALARM_SNAPSHOT_FILE|qt_alarm_snapshot\\.txt" "$SCRIPT_DIR/main.cpp"; then
    fail "main.cpp 不应继续使用固定 qt_alarm_snapshot.txt，必须每次生成时间戳诊断快照"
fi
