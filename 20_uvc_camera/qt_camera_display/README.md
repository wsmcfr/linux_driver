...
| 自动视觉居中路线 | 首页按 `开始` 且 F4 `START_CYCLE` ACK 后，QML 启动 `autoVisionTimer`，每 100ms 通过 overlay socket 发送 `LOCATE`；`uvc_kms_overlay` 读取原始 YUYV 中水平居中的 300px 宽全高搜索带，使用亮度连通域提前发现从画面上方进入的零件，并返回零件中心和 bbox；MP157 使用上方来料规则，把 `center_y` 作为 `VISION_POS.axis_px`、`height/2` 作为 `target_px` 下发 F4；连续 3 帧满足 `abs(center_y-height/2)<=24px` 后发送 `BELT_STOP_CENTERED hold_ms=2000`，收到 ACK 后等待 2 秒并复用现有 `检测` 链路运行双模型；如果黑色波形零件已经识别过，后续短暂漏检只保持停机等待重新识别，不再发送会让 F4 重新扫描的 `VISION_LOST reason=1`。 |
...
| 手动控制路线 | 左侧 `手动控制` 页面提供传送带、检测辅助、安全状态、人工复核和命令日志；传送带按钮仍显示巡航/停止/查询状态等语义，但底层通过 `/dev/ttySTM2` 向 F407 USART1 下发二进制 `BELT_MANUAL_CONTROL` 或 `QUERY_STATUS` 帧，F407 再由 `UART4 PC10/PC11` 控制地址 `0x01` 的张大头 Emm42 传送带电机；相机上下轴和前后轴共用 F407 `USART6 PC6/PC7`，当前首页不直接提供点动按钮，后续必须先由 F407 提供二进制命令、状态回读和失败码后再接入。 |
| 参数设置路线 | 左侧 `参数设置` 页面已收敛为波形垫圈、平垫圈、弹性垫圈三种真实零件；`DetectSettingsController` 启动时读取 `/mnt/sdcard/config/defect_ui_config.json`，页面可真实调整模型阈值、复核阈值、ROI 大小、UNet 最小缺陷像素、overlay 透明度、COS 自动上传开关和三台步进电机参数；保存配置用 `QSaveFile` 原子写 JSON，下一次检测会把模型阈值传给 `defect-classify --bad-threshold`，把 ROI、overlay 透明度和最小缺陷像素传给 `defect-segment --roi/--alpha/--min-defect-pixels`，步进电机参数保存到 `stepper_motors` 数组；步进弹窗的 `保存并下发` 会在保存 JSON 后通过 `/dev/ttySTM2` 发送二进制 `STEPPER_PARAM_SET 0x42`，负载为 25 字节，包含 `cycle_id=0`、`motor_count=3`、`flags=0` 和三条 7 字节电机记录；`保存配置` 和 `导出摘要` 都会追加 `/mnt/sdcard/logs/qt_settings_YYYYMMDD.log`，日志查看页刷新后可点开看到 `config_path`、阈值、ROI、UNet 像素阈值、overlay、上传策略、三台电机地址/步长/速度/方向和模型命令参数；关闭自动上传时仍保存 source/annotated 和本地历史，最终返回 `upload_status=SKIP`；F4 返回 `ACK acked_cmd=0x42 status=0` 只表示 F4 运行内存接收参数，不代表 F4 已重新烧录，也不会写 Emm42 EEPROM；称重标定 UI 发送二进制 `WEIGHT_CALIBRATE 0x30`，负载为 5 字节，包含 `cycle_id=0`、`known_weight_g=1~5000` 和 `flags=0`，F4 成功返回 `ACK`，未去皮、克重越界或采样异常返回结构化 `NACK`。 |
...
| 首页自动居中流程 | 开发板屏幕点击首页 `开始` | F4 ACK `START_CYCLE` 后传送带扫描，MP157 每 100ms 请求 `LOCATE`，识别到目标后发送 `VISION_POS`；零件从画面上方进入后，`center_y` 向 `height/2` 靠近；连续 3 帧进入 ±24px 后发送 `BELT_STOP_CENTERED`，ACK 后等待 2 秒自动进入模型检测；目标已经出现后若短暂返回 `has_target=0`，界面显示 `目标短暂丢失/连续丢失但保持停机`，不会再发 `VISION_LOST reason=1` 让 F4 继续扫描。 |
...
| 2026-07-03 | 新增上方来料自动视觉居中 | overlay 支持 `LOCATE` 内存定位；Qt C++ 支持 `VISION_POS/VISION_LOST/BELT_STOP_CENTERED`；QML 在 `START_CYCLE` ACK 后启动 100ms 自动视觉循环，使用 `center_y` 对齐 `height/2`，连续 3 帧进入 ±10px 后停传送带，ACK 后 2 秒自动触发双模型检测。 |
...
| F4 串口 `/dev/ttySTM2` | F4 控制器真实接入判断、称重标定和传送带高层控制 | Qt 后台线程按 115200 8N1 raw 模式发送 `A5 5A ... 6B` 二进制帧；周期心跳发送 `HEARTBEAT`，人工刷新不节流；参数设置页称重标定弹窗发送二进制 `WEIGHT_CALIBRATE 0x30`，手动页传送带按钮发送 `BELT_MANUAL_CONTROL` 或 `QUERY_STATUS`，由 F407 转成 `UART4 PC10/PC11` 上地址 `0x01` 的 Emm42 控制；正确只认 `ACK/STATUS_REPORT`，错误只认 `NACK/FAULT_REPORT`。 |
...
| 手动控制页面 | 调试级人工操作界面 | 传送带状态来自 F407 二进制 `ACK/STATUS_REPORT` 和 `manualCommandLog`；手动页打开时通过 `setOverlayVisible(false)` 隐藏 KMS 视频 plane；相机上下轴、前后轴、急停、限位和联锁仍归 STM32F4，MP157 不直接发送 Emm42 帧，也不在 F407 未实现二进制协议时显示假按钮或假状态。 |
...
| F4 传送带命令桥 | MP157 到 STM32F4 的高层命令桥 | 已在 `DeviceHealthController::sendF4BeltCommand()` 中实现二进制传送带命令；只开放 `BELT_MANUAL_SCAN`、`BELT_MANUAL_STOP` 和 `QUERY_STATUS` 三个按钮语义，底层分别发送 `BELT_MANUAL_CONTROL` 或 `QUERY_STATUS`。相机上下轴和前后轴后续应在 F407 先新增固定二进制服务，再由 MP157 侧扩展白名单。 |
...
下面功能先记为后续实机联调任务。当前已经真实接入传送带二进制 `BELT_MANUAL_CONTROL/QUERY_STATUS`；相机上下轴和前后轴必须先在 STM32F407 工程中新增对应二进制服务，再扩展 MP157 白名单，未接入前不在 Qt 界面显示按钮或占位状态。

| 优先级 | 待加功能 | 联调触发条件 | 实现要点 |
|---|---|---|---|
| 高 | 相机上下/前后轴 F4 服务 | F407 增加第二个 Emm42 串口服务，一个地址/通道负责上下移动，一个地址/通道负责前后移动时 | 建议 F407 暴露回零、移动、停止、查询等二进制命令；MP157 只发高层命令，不直接拼 Emm42 帧。 |
| 高 | 相机轴限位和回零状态 | 上下/前后轴安装限位或零位传感器后 | F407 负责限位去抖、回零过程、越界拒绝和错误码；QML 只显示 F4 返回的状态，不用本地变量假定已回零。 |
| 中 | 复位到安全状态 | F4 已支持停止传送带、停止相机运动轴和清联锁状态时 | 一键下发 F4 二进制高层安全复位命令，完成后通过 `QUERY_STATUS/STATUS_REPORT` 刷新传送带和后续相机轴状态；失败时保留 `NACK/FAULT_REPORT` 错误码并禁止继续动作。 |
...
| 低 | 参数设置页 | 传送带速度、相机轴步距等运动参数需要现场可调时 | 当前参数页已真实保存视觉检测配置；运动参数仍不能由 MP157 直接写 Emm42，必须先由 F407 定义高层二进制协议、限幅、互锁和状态回读。 |
...
| 板端二进制功能标记 | 开发板 SSH | `strings /root/qt_camera_display/qt_camera_display \| grep -E 'sendF4VisionPosition|autoVisionTimer|BELT_STOP_CENTERED|requestAutoVisionLocate'` | 能看到自动视觉和 F4 视觉命令 marker，证明 QML/C++ 已重新编进当前板端 Qt 二进制。 | 若 marker 不存在，说明只改了源码或只拷了 QML，未重新交叉编译并替换 `/root/qt_camera_display/qt_camera_display`。 |
| 首页自动视觉居中 | 开发板屏幕 + F4 串口日志 | 点击首页 `开始`，把零件从画面上方放上传送带进入 ROI | 底部提示先出现 `自动流程ACK`，随后出现 `自动视觉：x=... y=... target=... error=... stable=...`，右侧偏差显示真实 `errorY`；F4 收到 `VISION_POS` 后误差逐步变小；连续居中后 Qt 显示 `F4已确认居中停机，2秒后开始模型检测`，传送带停止并自动触发双模型检测；目标出现后如黑色波形零件漏检，界面应显示 `目标短暂丢失` 或 `连续丢失但保持停机`，F4 不应收到新的 `VISION_LOST reason=1` 扫描命令。 | 若 F4 ACK 但电机不动，查传送带 UART4、地址 `0x01`、电源和 F4 传送带任务；若 Qt 一直未定位，查 `LOCATE` 和 `AUTO_LOCATE_MIN_LUMA_DELTA`；若越调越远，反转 F4 误差到方向的映射，不在 MP157 偷偷反号。 |
