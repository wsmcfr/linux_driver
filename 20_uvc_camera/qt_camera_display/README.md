...
| 2026-07-04 | 修正手动前后轴停止键无响应 | MP157 新增 `sendF4ActuatorStopNow()`，停止键和模拟急停不再等待普通执行器命令 ACK，而是后台线程直接写入 `ACTUATOR_STOP` 二进制帧并 `tcdrain()` 后返回；该强制 STOP 不清 `m_f4CommandRunning`，避免干扰仍在等待 ACK 的普通命令线程；QML 监听 `ACTUATOR_STOP_NOW` 单独显示写入结果；静态测试禁止恢复“停止排队”等旧路径。 |
...
|---|---|---|---|
| 高 | 手动速度模式和上下轴固定步数确认 | F407 烧录包含 `ACTUATOR_POS_MOVE/ACTUATOR_STOP/ACTUATOR_VEL_MOVE` 的固件后 | 验证传送带和前后轴 `ACTUATOR_VEL_MOVE` 按一次持续运动、停止键立即写入 `ACTUATOR_STOP` 并结束运动；验证上下轴下降/上升分别执行 `zDownFixedSteps/zUpFixedSteps` 固定步数；确认 `actuator=0xFF` 能停止全部可停止执行器。 |
...
| 手动前后轴停止键 | 开发板屏幕 + F4 串口日志 | 手动控制页打开三轴弹窗，切到 `摄像头前后电机`，点 `前进` 或 `后退` 后马上点 `停止` | Qt 底部先显示持续运动已下发，再显示 `F4强制停止帧已写入`；F4 侧应收到 `ACTUATOR_STOP actuator=1`，摄像头电机服务日志出现 `Stop applied`，前后轴地址应为 `0x03`；即使上一条 `ACTUATOR_VEL_MOVE` ACK 慢，停止帧也应先写入串口。 | 若 Qt 没有 `ACTUATOR_STOP_NOW` 文案，说明板端仍是旧二进制；若 F4 没有 `Stop applied`，查 `/dev/ttySTM2`、F4 协议是否烧录、USART6 接线、前后轴地址 `0x03`；若日志显示 stop 到了但电机不停，查张大头驱动器停止帧 `[03 FE 98 00 6B]`、电源和共地。 |
