...
| 自动视觉居中路线 | 首页按 `开始` 且 F4 `START_CYCLE` ACK 后，QML 启动 `autoVisionTimer`，每 100ms 通过 overlay socket 发送 `LOCATE`；`uvc_kms_overlay` 读取原始 YUYV 中水平居中的 300px 宽全高搜索带，使用亮度连通域、密度范围、中心孔采样、暗像素占比、贴边面积和孔背景对比过滤黑色传送带误识别，并返回零件中心和 bbox；MP157 使用上方来料规则，把 `center_y` 作为 `VISION_POS.axis_px`、`height/2` 作为 `target_px` 下发 F4，由传送带完成上料和前后居中；传送带 SCAN 使用参数页 `scan_speed_rpm`，零件入画后的 TRACK/短步微调使用 `normal_speed_rpm`，自动流程不再固定限制到 40rpm；连续 3 帧满足 `abs(center_y-height/2)<=24px` 后发送 `BELT_STOP_CENTERED`；随后下发 `ACTUATOR_POS_MOVE actuator=2 direction=DOWN` 让上下轴下降固定步数，C++ 会先等 ACK，再等同一 `related_seq` 的 F4 `EVENT_REPORT actuator-move-done`，其中 `status=0(reached-ack)` 表示 F4 收到张大头主动到位回包，`status=5(estimated-done)` 表示 F4 按速度、步数和安全余量估算完成；如果 F4 事件没有及时回来，MP157 会从本次实际发送的 `ACTUATOR_POS_MOVE` 帧内读取 `speed_rpm` 和 `steps`，计算 `mp157-local-estimated-done` 本地兜底完成，因此参数页修改速度、微调步数或 Z 轴下探/回升步数后，下一次命令的估算等待时间会同步变化；QML 拿到 F4 完成事件或 MP157 本地兜底完成详情后才复查 ROI；复查时 `errorY` 超出死区用传送带 `ACTUATOR_POS_MOVE actuator=0` 做前后短步微调，`errorX` 超出死区用摄像头左右轴 `ACTUATOR_POS_MOVE actuator=1` 做左右短步微调；确认 X/Y 都在 ROI 检测中心后才进入 `focus-settle` 等待约 3 秒，再复用现有 `检测` 链路运行双模型；模型检测完成后自动下发 `ACTUATOR_POS_MOVE actuator=2 direction=UP`，同样等待 F4 DONE 或 MP157 本地兜底完成后再下发 `MODEL_READY/ARM_JOB_START` 通知 F4 和 ESP32S3 机械臂。 |
