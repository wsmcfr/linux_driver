...
| 首页自动视觉居中和 Z 轴对焦等待 | 开发板屏幕 + F4 串口日志 | 点击首页 `开始`，把零件从画面上方放上传送带进入 ROI | 底部提示先出现 `自动流程ACK`，随后出现 `自动视觉：x=... y=... target=... error=... stable=...`，右侧偏差显示真实 `errorY`；F4 收到 `VISION_POS` 后误差逐步变小；连续居中后 Qt 发送 `BELT_STOP_CENTERED`，传送带停止；随后 Qt 发送 `ACTUATOR_POS_MOVE actuator=2 direction=DOWN`，收到 ACK 后显示 `等待约3秒让摄像头对焦稳定`，3 秒后才 ROI 复查并触发双模型检测；目标出现后如黑色波形零件漏检，界面应显示 `目标短暂丢失` 或 `连续丢失但保持停机`，F4 不应收到新的 `VISION_LOST reason=1` 扫描命令。 | 若 F4 ACK 但电机不动，查传送带 UART4、地址 `0x01`、电源和 F4 传送带任务；若 Z 轴不动，查 `ACTUATOR_POS_MOVE` 分发、USART6、上下轴地址 `0x03`、方向和供电；若 Qt 一直未定位，查 `LOCATE` 和 `AUTO_LOCATE_MIN_LUMA_DELTA`；若越调越远，反转 F4 误差到方向的映射，不在 MP157 偷偷反号。 |
