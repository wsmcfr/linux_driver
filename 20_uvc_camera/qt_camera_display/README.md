...
| F4 二进制心跳、状态和故障回包 | F4 健康心跳从每 8 秒尝试发送改为 120 秒节流，减少 RS485 占用；参数设置页新增 `称重标定` 弹窗，快捷克重 100/500/1000/2000g，也支持通过内置数字键盘输入任意 1~5000g 整数；Qt 不再写 `CAL` 文本，当前只发送二进制占位命令并按 `ACK/NACK/FAULT_REPORT` 显示结果。 | 本地执行 `"C:/Program Files/Git/bin/bash.exe" ./test_qt_kms_overlay_assets.sh`；板端点击 `参数设置` -> `称重标定`，用 `清空`、数字键和 `退格` 输入非快捷值如 `750` 后点击 `发二进制`；当前 F4 未实现称重标定二进制命令时，预期弹窗显示 `NACK ERR_CMD_UNKNOWN`。 |
...
| F4 串口 `/dev/ttySTM2` | F4 控制器真实接入判断、称重标定占位和传送带高层控制 | Qt 后台线程按 115200 8N1 raw 模式发送 `A5 5A ... 6B` 二进制帧；周期心跳发送 `HEARTBEAT`，人工刷新不节流；参数设置页称重标定弹窗当前只发送二进制占位 `MODEL_READY` 并展示 `NACK` 结构化错误；手动页传送带按钮发送 `BELT_MANUAL_CONTROL` 或 `QUERY_STATUS`，由 F407 转成 `UART4 PC10/PC11` 上地址 `0x01` 的 Emm42 控制；正确只认 `ACK/STATUS_REPORT`，错误只认 `NACK/FAULT_REPORT`。 |
...
| F4 称重标定弹窗 | 开发板屏幕 | 点击左侧 `参数设置`，在 `F4接入边界` 卡片点击 `称重标定`，选择 `1000 g` 或用弹窗数字键盘按 `清空`、`7`、`5`、`0` 输入 `750`，再点击 `发二进制` | 当前 Qt 不发送 `CAL` 文本，只发送二进制占位命令；F4 未实现正式标定命令时，弹窗和底部提示应显示 `F4: 标定命令失败：NACK ERR_CMD_UNKNOWN...`；后续正式实现标定命令后，成功也必须显示二进制 `ACK/STATUS_REPORT` 摘要。 | 若弹窗仍显示 `正在发送 CAL` 或 `[OK][WEIGHT]`，说明板端还是旧 QML/旧二进制；若无回包，先按 F4 串口握手状态排查；若输入被拒绝，确认克重为 1~5000 的整数；若按输入框不弹键盘，直接使用弹窗内置数字键盘。 |
| F4 称重标定 SSH 对照 | 开发板 SSH | `strings /root/qt_camera_display/qt_camera_display | grep -E 'MODEL_READY|ERR_CMD_UNKNOWN|formatF4ToastText|发二进制'` | 板端二进制包含二进制标定占位命令和 `F4:` 提示格式相关标记，证明 QML/C++ 已经重新编进程序。 | 该对照只证明部署版本正确，不直接向 F4 写 ASCII；若需要裸串口验证，应使用能发送二进制帧的专用工具，不能再用 `CAL 1000` 文本判断主链路。 |
...
| F4 称重标定 | 屏幕弹窗观察；源码静态检查 `rg -n "MODEL_READY|sendF4Command|formatF4ToastText" main.cpp qml/Main.qml` | QML 仍保留克重输入 UI，但当前 C++ 只发送二进制占位命令，不再向 `/dev/ttySTM2` 写 `CAL` 文本；F4 回包经 `f4CommandFinished` 进入弹窗和底部提示 | 未实现正式标定命令时应显示 `F4: 标定命令失败：NACK ERR_CMD_UNKNOWN...`；后续实现后也必须以二进制 `ACK/NACK/STATUS_REPORT/FAULT_REPORT` 显示，QML 主线程不阻塞。 |
