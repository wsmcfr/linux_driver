...
| 手动控制路线 | 左侧 `手动控制` 页面提供传送带、检测辅助、安全状态、人工复核和命令日志；手动电机控制入口打开 `manualMotorPopup` 三页弹窗，第一页控制传送带，第二页控制摄像头左右轴，第三页控制摄像头上下轴；传送带和左右轴方向按钮通过 `/dev/ttySTM2` 向 F407 下发 `ACTUATOR_VEL_MOVE`，点击一次持续运动，停止按钮通过 `sendF4ActuatorStopNow()` 直接写入 `ACTUATOR_STOP` 帧并 `tcdrain()`，不等待上一条普通运动 ACK，并由 `manualActuatorStopRetryTimer` 在短延时后补发一次 STOP，避免左右轴速度模式或上下轴位置模式现场停不下来；左右轴按钮显示为 `左移/右移`，传送带按钮仍显示 `后退/前进`；上下轴方向按钮仍下发 `ACTUATOR_POS_MOVE`，下降取 `zDownFixedSteps`，上升和 `回原位` 取 `zUpFixedSteps`，每点一次只走对应固定步数；模拟急停下发 `ACTUATOR_STOP actuator=0xFF`，同样走强制写入通道并补发；传送带仍保留 `BELT_MANUAL_CONTROL/QUERY_STATUS` 巡航、停止和状态查询语义；安全状态区域使用 `manualSafetyFlickable` 可滑动查看，避免底部按钮遮挡长文本。 |
...
| 修改原因 | 参数页 `设当前位置为零点` 的真实语义是 F4/Emm42 把当前位置作为新零点，不会主动移动；旧 QML 的 `回原位` 仍固定发送 `zUpFixedSteps`，所以设零后再点还会继续上升。手动停止键还需要取消 QML 本地旧 pending，避免旧运动 ACK 覆盖停止提示；C++ 强制 STOP 写入改为重复写入同一 STOP 帧，QML 再短延时补发一次 STOP，提高停止帧穿过普通运动 ACK 等待窗口的可靠性。 |
...
| 停止策略 | 手动弹窗的上下轴、左右轴和全部停止仍调用 `sendF4ActuatorStopNow()`；Qt 会立即清空旧的手动 pending 命令，C++ 会把同一 STOP 帧重复写入 3 次并 `tcdrain()`，返回详情包含 `repeat=3`；随后 `manualActuatorStopRetryTimer` 延时约 260ms 再补发一次 STOP，覆盖上一条运动帧刚启动时的串口时序窗口。如果 STOP 打断上下轴位置运动，Qt 会把上下轴本地零点可信标志清掉，要求操作者在当前位置重新设零后再用 `回原位`。 |
...
| 2026-07-07 | 修复手动停止键和参数范围 | `Main.qml` 给上下/左右停止键新增 `manualActuatorStopRetryTimer` 延时补发 STOP；最小步长按钮改为 `步长-50/步长+50`；`main.cpp` 和 QML 把机械臂等待超时上限从 `180000ms` 提高到 `300000ms`。 |
