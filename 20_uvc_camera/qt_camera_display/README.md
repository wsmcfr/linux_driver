...
| 参数设置路线 | 左侧 `参数设置` 页面已收敛为波形垫圈、平垫圈、弹性垫圈三种真实零件，只保留模型阈值、复核阈值、相机/光源/SD/COS 策略和 F4 接入边界说明；`视觉检测策略` 和 `F4接入边界` 卡片支持 `查看详情`，通过 `settingsDetailOverlay` 滚动查看 MP157 云端上报、source/annotated 图片、断网补传、F4 串口和传感器边界；`F4接入边界` 卡片新增 `称重标定`，弹窗内置屏幕数字键盘，可输入 1~5000g 并发送 `CAL <克重>\r\n`；`相机、光源与存储` 不再显示补光项，因为当前没有补光计划；除称重标定外，当前参数仍是 QML 本地 UI目标值，尚未写JSON，尚未下发F4。 |
| 告警维护路线 | 左侧 `告警维护` 页面提供当前告警、设备健康、告警历史、处理建议、确认/清故障/刷新/保存诊断入口；处理建议面板只显示短摘要，点击 `查看全部` 后进入 `alarmAdviceDetailOverlay`，由 `alarmAdviceDetailFlickable` 滚动查看相机、SD 卡、4G、云端、F4、保存/上传和模型链路完整排查说明；设备健康矩阵中的 `4G` 格直接绑定 `DeviceHealthController::networkStatusText`，`定位` 格直接绑定 `DeviceHealthController::locationStatusText`，不再显示参数配置占位；QML 监听 `DeviceHealthController` 和检测/上传返回值，运行中识别相机/KMS 无帧、SD 卡未挂载或不可写、4G/云端异常、F4 心跳异常、保存/上传失败、模型检测失败等真实问题；每个新问题调用 C++ 写入 `/mnt/sdcard/logs/qt_alarm_YYYYMMDD_HHMMSS_zzz_<source>.log`，每次 `保存诊断` 写入新的 `/mnt/sdcard/logs/qt_alarm_snapshot_YYYYMMDD_HHMMSS_zzz.txt`，两类文件都执行 `flush` 和 `fsync`，不覆盖固定快照文件。 |
| 真实健康状态 | `DeviceHealthController` 通过 `QTimer` 每 8 秒周期调度，4G/云端/4G IP 省份定位用异步 `QProcess`，相机 `STATUS` 和 F4 串口握手用后台线程；4G/云端/定位进程启动失败由 `errorOccurred` 信号回写状态，不在刷新路径调用 `waitForStarted()`；定位开机第一轮通过 `4g-location once` 调用高德 IP 定位，后续每轮只通过 `4g-location status` 读取缓存，顶部栏只显示省份，例如 `河南省`；F4 `STATUS\r\n` 后台心跳按 120 秒节流发送，人工刷新和打开标定弹窗时可立即触发一次；周期刷新保留上一轮稳定状态，不再每轮把网络/云端/位置显示成“检测中”；QML 只绑定状态属性，不执行 socket、串口、GPS 或 shell 阻塞调用。 |
...
| 顶部位置和告警定位格 | `DeviceHealthController` 新增 `locationStatusText/locationDisplayText/locationShortText/locationStatusColor`；开机第一轮异步调用 `4g-location once` 获取高德 IP 省份定位，之后只调用 `4g-location status` 读缓存；顶部栏用 `位置:<省份>` 替换静态机械臂状态，告警健康矩阵用 `定位` 格显示 IP 定位、缺少Key或失败。 | 板端配置 `AMAP_WEB_KEY` 或 `/etc/4g-location/amap-web-key` 后执行 `4g-location once; cat /var/run/4g-location.state`，屏幕顶部应显示类似 `位置:河南省`，告警页定位格显示 `IP定位`；缺少 Key 时显示 `缺少Key`。 |
| 参数详情完整显示 | 参数设置页的 `视觉检测策略` 和 `F4接入边界` 卡片新增 `查看详情`；视觉详情按云端上报契约说明 `record_no`、零件归一、双模型判定、`source/annotated` 图片、COS 登记和断网补传；F4 详情说明 `/dev/ttySTM1`、`115200`、心跳/CRC/帧序号、LDC1614、HX711、Emm42_V5.0、急停限位和 Qt 不接管运动控制的边界。 | 本地执行 `"C:/Program Files/Git/bin/bash.exe" ./test_qt_kms_overlay_assets.sh`；板端点击 `参数设置` -> `视觉检测策略/查看详情` 和 `F4接入边界/查看详情`，确认弹层可滚动读完；确认 `相机、光源与存储` 不再显示 `补光`。 |
...
| F4 心跳与称重标定 | F4 健康心跳从每 8 秒尝试发送改为 120 秒节流，减少 RS485 占用；参数设置页新增 `称重标定` 弹窗，快捷克重 100/500/1000/2000g，也支持通过内置数字键盘输入任意 1~5000g 整数，发送 `CAL <克重>\r\n` 给 F407 的 HX711 标定服务；F4 成功或失败回包保留较完整文本，弹窗结果区支持多行滚动查看，避免 `[OK][WEIGHT] Calibration success...` 被单行省略。 | 本地执行 `"C:/Program Files/Git/bin/bash.exe" ./test_qt_kms_overlay_assets.sh`；板端点击 `参数设置` -> `称重标定`，用 `清空`、数字键和 `退格` 输入非快捷值如 `750` 后点击 `发送CAL`；也可 SSH 执行 `/root/uartapp /dev/ttySTM2 115200 send "CAL 750\r\n"` 对照。 |
| 4G 定位开机一次 | Qt 周期健康刷新仍保持 8 秒，但定位链路只在开机第一轮执行一次 `4g-location once`；后续全部读取 `/var/run/4g-location.state` 缓存，不再反复调用高德接口，也不访问 GPS/AT 串口。 | 本地执行 `"C:/Program Files/Git/bin/bash.exe" ./test_qt_kms_overlay_assets.sh`；板端执行 `strings /root/qt_camera_display/qt_camera_display | grep -E 'm_locationBootProbeDone|locationShortText|位置'`，再看 `/var/log/4g-location.log` 不应每 8 秒重复出现 `amap ip rc=`。 |
...
| 参数设置页真实零件 | 参数页 `零件与模型判定` 只在波形垫圈、平垫圈、弹性垫圈之间循环；删除旧演示零件名和会误导用户的运动/分拣可调项，保留模型阈值、复核阈值、相机/光源/存储策略和 F4 接入边界。 | 虚拟机或本地执行 `sh ./test_qt_kms_overlay_assets.sh`；板端打开 `参数设置`，确认界面显示 `UI目标值`、`尚未写JSON`、`尚未下发F4`，点击切换时只出现三种真实零件。 |
| 真实告警日志与诊断快照 | 告警维护页不再依赖固定演示告警；相机/KMS、SD、4G、云端、F4、保存/上传和模型链路出现新问题时写独立 `qt_alarm_*.log`，每次点击 `保存诊断` 或执行自检都写新的 `qt_alarm_snapshot_*.txt`，不覆盖旧文件。 | 虚拟机执行 `./test_qt_kms_overlay_assets.sh`；板端执行 `--alarm-snapshot-self-test`、`--alarm-log-self-test` 后用 `ls -lt /mnt/sdcard/logs/qt_alarm*.{log,txt}`、`test -s`、`tail` 验证文件非空并含来源、状态和排查建议。 |
...
| 路径 | 修改原因 |
|---|---|
| `20_uvc_camera/qt_camera_display/main.cpp` | 注册 `V4L2VideoItem`，向 QML 注入摄像头节点、采集尺寸、视频后端、GStreamer 状态等上下文参数；`CameraStorageController` 负责请求 overlay 保存当前帧、串行调用 `defect-classify` 和 `defect-segment`、通过 `detectClassificationReady` 在第一个模型结束后立即通知 QML 刷新零件类型和类别、通过 `detectModelsReady` 在两个模型结束后立即通知 QML 刷新 `fused_status/fused_result/fused_reason` 与 `total_time_ms`、按分类和 UNet 综合结果设置 `CLOUD_RESULT=good/bad/review`，并把分类 `class=gasket_good/gasket_bad` 统一剥离成 `CLOUD_PART_CODE=gasket` 后调用 `defect-cos-upload --jpg <source> --annotated <raw> --annotated <overlay> --annotated <mask>` 上传本次检测图片、调用 `sdcard-safe-remove`；`saveAlarmSnapshotToSdCard()` 每次生成新的 `qt_alarm_snapshot_*.txt` 诊断快照，`recordAlarmIssueToSdCard()` 为每个新问题生成新的 `qt_alarm_*.log` 自动告警日志，两者共用目录创建、挂载/可写检查、UTF-8 写入、`flush` 和 `fsync` 逻辑；`DeviceHealthController` 每 8 秒周期异步探测 4G、4G IP 省份定位、相机、F4、云端和 SD 卡真实状态，4G/云端/定位进程启动失败通过异步错误信号回写，不在健康检测刷新路径等待启动，周期刷新保留网络/云端/位置上一轮稳定状态；定位开机第一轮解析 `4g-location once` 输出，后续只解析 `4g-location status` 缓存输出，向 QML 暴露 `locationStatusText/locationDisplayText/locationShortText/locationStatusColor`；F4 `STATUS\r\n` 心跳按 120 秒节流，`sendF4Command()` 可后台发送 `CAL <克重>\r\n` 并把 F407 回复通过 `f4CommandFinished` 回传 QML；摄像头连续离线时只调用 `restart-overlay` 重启 overlay 视频进程，SD 卡状态会区分未挂载和不可写；提供 `--detect-self-test`、`--storage-self-test`、`--alarm-snapshot-self-test` 和 `--alarm-log-self-test` SSH 自检入口；`UploadHistoryModel` 把每次检测结果追加到 `/mnt/sdcard/images/upload_history.json` 并暴露给 QML 历史页，同时提供 `removeRecord()` 删除历史记录和对应 source/annotated 图片文件；`retryUploadRecord()` 可复用已有本地 source/annotated 图片重新上传失败记录，重发时同样综合 `classification_result` 和 `segmentation_result` 恢复云端结果和零件类型，成功后把该条本地历史的 `upload_time` 改成本次重发时间、更新 `upload_status/record_id/record_no`，并移动到历史数组末尾作为最新记录。 |
| `20_uvc_camera/qt_camera_display/qml/Main.qml` | 实现 1024x600 工业检测界面、状态栏、结果面板、统计区、`开始/暂停/继续/停止` 触摸按钮，以及 `检测`、`安全卸载` 两个真实操作按钮；顶部网络、相机、F4、位置、云端和告警页设备健康矩阵绑定 `deviceHealth`，4G 测试不通过不显示在线，4G IP 省份定位缺少 Key 或接口失败时不伪装成已定位，F4 未串口握手成功不显示接入，KMS 相机未出帧或帧序号停滞不显示在线；告警页设备健康矩阵的 `4G` 格直接绑定 `deviceHealth.networkStatusText`，`定位` 格直接绑定 `deviceHealth.locationStatusText`，处理建议面板通过 `查看全部` 打开 `alarmAdviceDetailOverlay`，完整建议由 `alarmFullAdviceText()` 生成并通过 `alarmAdviceDetailFlickable` 滚动阅读；相机恢复在线时，`onCameraStatusChanged` 必须同时满足 `bootOverlayRestoreFinished`、`!splashOverlayVisible` 和当前在首页，才会发送 `VISIBLE 1` 恢复 overlay 画面，避免健康检测先于 Qt 启动画面把摄像头层打开；返回首页的 `switchPage()` 路径也使用同一启动完成门控，不在首页则保持隐藏；`检测` 调用异步 `requestDetectCurrentFrame()`，检测期间显示 `检测中...`，收到 `detectClassificationReady` 后立即显示模型识别出的零件类型、短类别、分类初判和置信度，但主结果显示“等待综合判定”，收到 `detectModelsReady` 后用综合判定覆盖首页主状态并显示双模型总耗时，最终 `detectCurrentFrameFinished` 只恢复忙状态并显示上传/历史记录结果；首页窄结果栏通过 `compactHomeClassText()` 和 `compactHomeModelText()` 显示短类别和短模型结论，`gasket/gasket_good` 显示为 `波形垫圈`，`washer` 显示为 `平垫圈`，`splitwasher` 显示为 `弹性垫圈`；参数设置页使用 `settingsSupportedPartTypes` 固定三种真实零件，`零件与模型判定` 只保留模型阈值和复核阈值，`视觉检测策略` 展示分类、UNet 复核、低可信度和 ROI 策略并通过 `settingsDetailOverlay` 查看 `record_no`、`source/annotated`、断网补传和云端记录契约，`F4接入边界` 通过同一弹层查看 `/dev/ttySTM1`、LDC1614、HX711、Emm42_V5.0、急停限位和 Qt 不接管运动控制的说明；两个参数详情按钮使用底部右侧预留区域，摘要列表压缩为 4 行，底部状态文字在左侧显示，避免按钮遮挡文本；每次打开参数详情都会把 `settingsDetailFlickable.contentY` 重置为 0，避免两个详情复用同一 Flickable 时继承上次滚动位置；`相机、光源与存储` 移除补光行，不再暗示现场已有补光计划；告警维护页通过 `evaluateRuntimeAlarms()` 监听相机/KMS、SD 卡、4G、云端、F4、保存/上传和模型检测状态，新问题调用 `recordAlarmIssueToSdCard()` 写时间戳告警日志，`保存诊断` 调用 `saveAlarmSnapshotToSdCard()` 写时间戳诊断快照；历史、统计、手动和启动动画等既有页面继续保持原有交互。 |
| `20_uvc_camera/qt_camera_display/README.md` | 同步记录告警处理建议 `查看全部` 弹层、设备健康 `4G` 状态格、顶部位置显示、告警页 `定位` 状态格、修改原因、板端验证命令和失败排查方式，避免后续只看文档时仍以为右下角建议只能显示短文本、最后一格仍是参数配置或顶部没有城市区县。 |
...
| `20_uvc_camera/qt_camera_display/defect-cos-upload` | 板端 COS 上传脚本，先读取 `/root/qt_camera_display/cos-upload.env` 作为默认上传账号；`--jpg` 原图按 `file_kind=source` 上传，重复传入的 `--annotated <jpg/png>` 检测结果图按 `file_kind=annotated` 上传，并按扩展名自动使用 `image/jpeg` 或 `image/png`；`CLOUD_RESULT` 只接受 `good/bad/review`，检测链路必须显式传入模型结果，避免坏品记录被固定写成良品；`CLOUD_PART_CODE` 表示零件类型，脚本先按 `/api/v1/parts?limit=100` 查询云端 `part_id`，查不到时不再失败，而是在创建记录请求中发送 `part_code/part_name/part_category/auto_create_part=true` 让云端自动创建零件；`gasket/wave_washer` 自动写入 `part_name=波形垫圈`、`part_category=垫圈类`，`washer` 写入 `平垫圈 / 垫圈类`，`splitwasher` 写入 `弹性垫圈 / 垫圈类`，可用 `CLOUD_PART_NAME/CLOUD_PART_CATEGORY` 覆盖。旧 `--png` 参数仍兼容，内部等同于一张 `--annotated` 结果图。 |
...
| `20_uvc_camera/qt_camera_display/test_qt_kms_overlay_assets.sh` | 静态检查 overlay 源码、构建脚本、运行脚本、触摸按钮、SD 卡按钮、历史记录模型/页面、历史检测信息紧凑显示、首页结果短字段、失败历史重新发送、统计分析页面、统计分布两列布局、手动控制页面、参数设置页面、参数详情弹层、补光项移除、告警维护页面、处理建议完整弹层、4G 健康状态格、真实设备健康状态、健康检测非阻塞契约、时间戳告警日志/诊断快照落盘接口、早期静态首帧绘制器、启动动画覆盖层、QML 资源不压缩打包和默认启动契约。 |
| `docs/plans/2026-05-21-qt-alarm-advice-detail-implementation.md` | 记录告警处理建议完整说明和 4G 健康状态格的实施计划、测试先行步骤、QML 改动、文档更新和验证命令。 |
| `docs/plans/2026-05-21-qt-settings-detail-implementation.md` | 记录参数设置页视觉检测策略详情、F4 接入边界详情、补光项移除、静态测试先行、QML 改动、文档更新和验证命令。 |
...
| 打开参数设置 | 开发板屏幕点击左侧 `参数设置` | 实时视频 plane 被临时隐藏；页面显示 `零件与模型判定`、`视觉检测策略`、`F4接入边界`、相机光源与存储、参数摘要和操作按钮；页面明确显示 `UI目标值`、`尚未写JSON`、`尚未下发F4`。 |
| 调整参数设置 | 开发板屏幕点击 `参数设置` 页中的 `+/-`、`切换`、`应用到UI`、`保存预留`、`恢复默认` | 只允许调模型阈值、复核阈值和 COS 自动/本地策略；零件只在波形垫圈、平垫圈、弹性垫圈之间循环；界面参数值、参数摘要和底部提示条同步变化；当前不写真实配置文件，不向 F4 下发参数。 |
| 打开告警维护 | 开发板屏幕点击左侧 `告警维护` | 实时视频 plane 被临时隐藏；页面显示真实当前告警或 `ALM-OK/ALM-INIT` 占位、设备健康矩阵、告警历史和处理建议。 |
| 告警维护操作 | 开发板屏幕点击 `确认`、`清故障`、`刷新状态`、`保存诊断` | 当前告警状态或维护日志更新；点击 `保存诊断` 时底部提示条显示新的 `诊断已保存：/mnt/sdcard/logs/qt_alarm_snapshot_*.txt` 或明确失败原因；清故障只改变维护状态，不代表真实 F4 联锁已经解除。 |
| SSH 查看告警日志和诊断快照 | 开发板屏幕点击 `保存诊断` 后，或执行 `--alarm-snapshot-self-test`/`--alarm-log-self-test` 后，在 SSH 执行 `ls -lt /mnt/sdcard/logs/qt_alarm*.log /mnt/sdcard/logs/qt_alarm_snapshot_*.txt 2>/dev/null | head; file=$(ls -t /mnt/sdcard/logs/qt_alarm_snapshot_*.txt | head -n 1); test -s "$file"; wc -c "$file"; tail -n 30 "$file"` | 最新快照文件存在且大小大于 0，内容包含 `STM32MP157 Qt Alarm Snapshot`、`alarm_code=`、`device_health=`、`storage_state=` 和 `[recent_alarm_history]`；自动告警日志 `qt_alarm_*.log` 含 `source=`、问题描述和 `[troubleshooting]`。 |
...
| 2026-05-20 | 参数设置页真实零件收敛 | `settingsSupportedPartTypes` 只保留波形垫圈、平垫圈、弹性垫圈；参数页删除旧演示零件名和容易误导的运动/分拣可调项，保留模型阈值、复核阈值、相机/光源/SD/COS 策略，并在界面说明当前只是 QML UI目标值，尚未写JSON，尚未下发F4。 |
...
| 2026-05-18 | 顶部和健康矩阵改为真实设备状态 | 新增 `DeviceHealthController`，4G 必须 `4g-ppp test` 返回 0 才显示在线；KMS 相机必须 overlay `STATUS` 返回 `has_frame=1`、`serial>0` 且 `serial` 相对上一轮有变化才显示在线；F4 必须 `/dev/ttySTM2` 发送 `STATUS\r\n` 并收到 `ACK/OK/F4/READY` 才显示接入；云端必须 `curl -fsS --max-time 2 http://139.9.35.72/health` 成功才显示已连接；4G/云端启动失败通过 Qt 异步错误信号处理，不在刷新路径等待进程启动；健康刷新周期放慢到 8 秒，并保留上一轮稳定状态，避免网络/云端在“检测中”和“在线/已连接”之间循环闪烁；所有耗时检测都用异步进程或后台线程，QML 主线程不等待。 |
...
| 4G IP 省份定位命令 `4g-location` | 顶部位置和告警页定位状态 | Qt 开机第一轮异步调用 `4g-location once`，之后每 8 秒只调用 `4g-location status` 读取缓存；成功时顶部只显示省份，例如 `位置:河南省`，失败时显示缺少Key、未定位或未安装。 |
| 高德 Web 服务 Key | 高德 IP 省份定位 | 通过 `AMAP_WEB_KEY` 或板端本地 `/etc/4g-location/amap-web-key` 提供，不提交 Git；`4g-location` 只调用高德 IP 定位接口，不使用 GPS 坐标转换或逆地理编码。 |
| `/var/run/4g-location.state` | 4G IP 省份定位缓存 | 保存 `state/display/short_display/province/city/district/longitude/latitude/satellites/updated_at/detail`；当前只写省份，城市、区县、经纬度和卫星数字段保持为空。 |
...
| F4 串口 `/dev/ttySTM2` | F4 控制器真实接入判断和 HX711 称重标定 | Qt 后台线程按 115200 8N1 raw 模式发送 `STATUS\r\n`；周期心跳按 120 秒节流，人工刷新不节流；参数设置页称重标定弹窗发送 `CAL <克重>\r\n`，回复中包含 `ACK`、`OK`、`F4` 或 `READY` 时认为串口命令成功，包含 `ERROR` 时显示失败详情。 |
...
| 4G IP 省份状态文件 | 开发板 SSH | `mkdir -p /etc/4g-location; test -s /etc/4g-location/amap-web-key || printf '%s\n' "$AMAP_WEB_KEY" > /etc/4g-location/amap-web-key; chmod 600 /etc/4g-location/amap-web-key; 4g-location once; echo "exit=$?"; cat /var/run/4g-location.state; tail -n 80 /var/log/4g-location.log` | 成功时状态文件包含 `state=ip_ok`、`display=河南省`、`short_display=河南省`、`province=河南省`，`city/district/longitude/latitude/satellites` 为空；日志能看到 `amap ip rc=0`。 | 若 `state=no_key`，先配置 `AMAP_WEB_KEY` 或 `/etc/4g-location/amap-web-key`；若 `state=ip_failed`，查 4G 网络、高德 Web 服务 Key、IP 定位接口权限和配额。 |
| Qt 顶部位置栏 | 开发板屏幕和 SSH | `4g-location status; strings /root/qt_camera_display/qt_camera_display | grep -E 'm_locationBootProbeDone|locationShortText|位置|定位'`，然后点击或等待健康刷新 | 顶部状态栏显示 `位置:河南省` 这类省份短文本；如果缺 Key 显示 `缺少Key`，IP 定位失败显示 `未定位`。 | 若状态文件正确但屏幕不变，确认 Qt 二进制包含 `m_locationBootProbeDone/locationShortText` 且服务已重启；若日志每 8 秒重复 `amap ip rc=`，检查 `startLocationProbe()` 是否又重复执行 `once`。 |
| 告警页定位健康格 | 开发板屏幕 | 点击左侧 `告警维护`，再点击 `刷新状态`，对照 SSH 执行 `cat /var/run/4g-location.state` | 设备健康矩阵 `定位` 格绑定 `deviceHealth.locationStatusText`：成功为 `IP定位`，缺 Key 为 `缺少Key`，失败为 `未定位`，且颜色随状态变化。 | 若仍显示旧的 `背光` 或 `配置` 占位，说明 QML 资源或 Qt 二进制未更新；若定位状态与状态文件不一致，查 `parseKeyValueOutput()` 和 `handleLocationProcessFinished()`。 |
...
| F4 串口握手状态 | 开发板 SSH | `test -e /dev/ttySTM2 && stty -F /dev/ttySTM2 115200 raw -echo -crtscts min 0 time 10; exec 3<>/dev/ttySTM2; printf 'STATUS\r\n' >&3; dd bs=80 count=1 <&3 2>/dev/null; exec 3>&-` | 读到包含 `ACK`、`OK`、`F4` 或 `READY` 的回复时，Qt 显示 `接入`；没有回复、串口不存在或回复不匹配时显示 `待接入`；该命令不依赖 `timeout`，适合当前 BusyBox 根文件系统。 | 若命令占用串口影响 Qt，先关闭手工文件描述符或重启 shell；若无回复，先确认 F407 已烧录包含 `STATUS` 回复的新固件，再查串口线、TX/RX 交叉、共地和波特率。 |
| F4 称重标定弹窗 | 开发板屏幕 | 点击左侧 `参数设置`，在 `F4接入边界` 卡片点击 `称重标定`，选择 `1000 g` 或用弹窗数字键盘按 `清空`、`7`、`5`、`0` 输入 `750`，放稳砝码后点击 `发送CAL` | 弹窗显示 `正在发送 CAL 750 ...`，随后在可滚动结果区显示 F407 `[OK][WEIGHT] Calibration success...` 或明确错误原因；长回包可在结果区上下滑动查看，底部提示条同步显示短结果；发送期间不会卡住页面触摸。 | 若显示 `Tare is not ready`，先确认 F407 启动去皮成功或通过串口执行 `TARE`；若显示超时，先用本表上一条 `STATUS` 验证 RS485；若输入被拒绝，确认克重为 1~5000 的整数；若按输入框不弹键盘，直接使用弹窗内置数字键盘；若回包很长，在弹窗结果框内上下滑动查看完整内容。 |
| F4 称重标定 SSH 对照 | 开发板 SSH | `/root/uartapp /dev/ttySTM2 115200 send "CAL 1000\r\n"` | stdout 显示 `[TX ...]`，F407 若能回复应通过接收模式或 Qt 弹窗看到 `[OK][WEIGHT]` 或 `[ERROR][WEIGHT]`。 | `send` 模式只证明 MP157 写出了字节；若要看回包，用 `term` 或关闭 Qt 后用 `recv`，避免多个进程同时打开同一串口造成读回包竞争。 |
...
| 历史页触摸验证 | 开发板屏幕 | 点击左侧 `历史记录`，确认停留在检测历史列表且最新卡片可见并高亮；横向滑动检测时间卡片，点击某条记录空白区域选中，再点击 `查看`，再在左侧图片区域左右滑动 | 进入历史页默认显示列表层，最新记录卡片可见；点击 `查看` 后进入详情页；详情页左侧能在原始图片、UNet raw、UNet overlay、UNet mask 之间切换，右侧只显示上传时间、记录ID、图片数量、云端编号、云端状态，以及“检测结论/可信度/缺陷提示/图片留档”四条普通中文说明，不单独显示本地路径。 | 若历史页空白，先查 `/mnt/sdcard/images/upload_history.json`；若点选后列表自动滚动，检查 `Main.qml` 是否又出现 `currentIndex: root.selectedHistoryIndex` 或 `highlightRangeMode: ListView.ApplyRange`；若图片不显示，确认文件路径存在且图片头正确；若无法触摸，查 Goodix 输入配置。 |
| 历史失败重发验证 | 开发板屏幕和 SSH | 准备一条 `upload_status` 含 `上传失败` 的历史记录，屏幕进入该记录详情点击 `重新发送`，再执行 `tail -n 120 /mnt/sdcard/images/upload_history.json` | 点击后按钮短暂显示 `发送中`；成功时同一条 JSON 记录移动到数组最后，`upload_time` 变为本次重发完成时间，并出现新的 `record_id`、`record_no` 和 `上传成功` 状态，界面详情同步刷新到这条最新记录；失败时该条记录保留原时间、原位置和失败状态并允许再次重发。 | 若按钮不显示，确认 JSON 中 `upload_status` 含 `失败`；若成功但界面不刷新，查 `retryUploadFinished` 和 `updateRecordUploadResult`；若时间仍是旧值，查 `refreshedUploadTime` 和 `m_entries.move(row, lastRow)`；若上传失败，查 4G、云端 health、账号配置、`/tmp/defect-cos-record*.json` 和脚本 stderr。 |
| 统计页触摸验证 | 开发板屏幕 | 点击左侧 `统计分析`，观察 KPI、最近检测趋势、分布概览、云端与文件状态、最近记录表；在最近记录卡片内上下滑动，再点击任一记录行 | 统计页打开后实时视频 plane 隐藏；总记录应等于 `upload_history.json` 记录数；良品/待复核、上传成功率、图片数量和文件大小有值；`分布概览` 左列显示良品/坏品/待复核，右列显示上传成功/上传失败，五条都在面板边框内；最近记录表能在卡片内竖向滑动查看更多记录；点击最近记录行进入对应历史详情页。 | 若统计页无数据，先查 `/mnt/sdcard/images/upload_history.json` 是否存在且非空；若分布概览仍越界，检查 `statsDistributionLeftColumn`、`statsDistributionRightColumn` 和 `statsDistributionBarDelegate`；若不能滑动，检查 `statsRecentListView` 是否仍是 `ListView`；若点击无反应，检查 `openHistoryDetailFromStats` 和 `showHistoryDetail`；若视频仍覆盖统计页，查 `setOverlayVisible(pageName === "home")` 和 overlay `VISIBLE` 命令。 |
...
| 参数页本地契约验证 | 开发板屏幕 | 点击 `应用到UI`、`保存预留`、`恢复默认`，观察顶部摘要和底部提示 | 页面明确提示 `UI目标值`、`尚未写JSON`、`尚未下发F4`；`保存预留` 只提示后续 JSON 路径，不生成配置文件，不触发 F4 串口动作。 | 若界面暗示已经保存或已经同步 F4，先查 `settingsApplyAction()` 文案；若后续新增持久化，必须同步 C++ 控制器、文件 fsync 和 F4 协议文档。 |
| 参数页布局验证 | 开发板屏幕 | 点击左侧 `参数设置`，依次观察顶部摘要、三张参数卡、下方存储卡和参数摘要卡；点击 `+/-`、`切换`、`应用到UI`、`保存预留`、`恢复默认` | 所有文字都在卡片边框内，长路径和摘要只省略不换行溢出；参数按钮不挤出卡片，底部提示条不遮挡其它区域。 | 若仍有文字越界，先查 `Main.qml` 中对应卡片是否缺少 `clip: true`、`elide: Text.ElideRight/ElideMiddle`，再按板端实际字体继续缩短文案或减少同屏字段。 |
...
| 告警诊断快照落盘 | 开发板屏幕和 SSH | 屏幕点击 `告警维护` 的 `保存诊断`，或执行 `/root/qt_camera_display/qt_camera_display --alarm-snapshot-self-test`，再执行 `file=$(ls -t /mnt/sdcard/logs/qt_alarm_snapshot_*.txt | head -n 1); test -s "$file"; wc -c "$file"; tail -n 30 "$file"` | 每次点击或自检都生成新的时间戳快照文件；文件存在且非空，`tail` 能看到 `snapshot_time=`、`alarm_code=`、`alarm_status=`、`camera_status=`、`device_health=` 和 `[recent_alarm_history]`；按钮提示为真实 C++ 返回值。 | 若目录为空，先查屏幕提示或自检输出是否为 `诊断保存失败`；再查 `/tmp/qt_camera_display.log` 中 `storage action alarm-snapshot`、`mount | grep ' /mnt/sdcard '`、`df -h /mnt/sdcard` 和 SD 卡是否只读。 |
| 自动告警日志落盘 | 开发板屏幕和 SSH | 触发相机/KMS 无帧、SD 卡不可写、4G/云端异常、F4 待接入、保存/上传失败或模型检测失败；也可执行 `/root/qt_camera_display/qt_camera_display --alarm-log-self-test`，再执行 `log=$(ls -t /mnt/sdcard/logs/qt_alarm_*.log | head -n 1); test -s "$log"; wc -c "$log"; tail -n 40 "$log"` | 每个新问题生成独立 `qt_alarm_*.log`，内容包含 `occurrence_time=`、`source=`、`alarm_code=`、`device_health=` 和 `[troubleshooting]`；同一问题未恢复前不会因 8 秒健康刷新重复刷日志。 | 若没有新日志，先确认问题是否已经在 `activeAlarmKeys` 中处于未恢复状态；再查屏幕提示、`/tmp/qt_camera_display.log` 中 `storage action alarm-log`、SD 卡挂载/可写状态和 C++ 返回原因。 |
...
| 高德 IP 省份定位 | `4g-location once; grep -E 'state=ip_ok|province=|display=|short_display=|city=|district=|longitude=|latitude=|satellites=' /var/run/4g-location.state` | `4g-location` 读取 `AMAP_WEB_KEY` 或 `/etc/4g-location/amap-web-key`，只调用高德 IP 定位接口，不打开 GPS 或 AT 串口 | 状态文件只显示省份；城市、区县、经纬度和卫星数字段为空，避免把运营商出口城市当成板子真实位置。 |
| Qt 位置状态绑定 | `strings /root/qt_camera_display/qt_camera_display | grep -E 'locationStatusText|locationDisplayText|locationShortText|m_locationBootProbeDone|LOCATION_PROBE_TIMEOUT_MS'` | `DeviceHealthController` 开机第一轮执行 `4g-location once`，后续周期执行 `4g-location status` 读缓存，解析 key=value 输出并通过 QML 属性显示 | 二进制包含定位属性、40 秒定位超时常量和开机一次定位 marker；界面顶部显示 `locationShortText`，告警页显示 `locationStatusText`，诊断摘要记录 `locationDisplayText`。 |
| F4 状态探测 | `ls -l /dev/ttySTM2`，必要时用串口工具读取 F4 回复 | Qt 后台线程向 `/dev/ttySTM2` 写入 `STATUS\r\n` 并等待短回复；常规心跳 120 秒节流，人工刷新立即发送 | 只有回复包含 `ACK/OK/F4/READY` 才显示接入；未通信成功时显示待接入。 |
| F4 称重标定 | 屏幕弹窗观察；SSH 执行 `/root/uartapp /dev/ttySTM2 115200 send "CAL 1000\r\n"` 对照 | QML 调用 `sendF4Command("CAL <克重>")`，C++ 后台线程自动补 `\r\n` 后写入 `/dev/ttySTM2` 并等待 F407 文本回复 | 回复含 `OK` 时弹窗显示成功；F407 回复 `ERROR`、无回复或串口失败时弹窗显示错误详情；较长回包在弹窗结果区多行显示并可上下滑动；QML 主线程不阻塞。 |
...
| 历史失败重发 | `tail -n 120 /mnt/sdcard/images/upload_history.json` | 历史详情页 `重新发送` 调用 `CameraStorageController::retryUploadRecord()`，后台复用本地 source/annotated 图片上传，主线程调用 `UploadHistoryModel::updateRecordUploadResult()` 更新 JSON | 重发不会新增本地图片文件；成功时同一条记录的 `upload_time` 改为重发完成时间，记录移动到数组末尾，`record_id/record_no/upload_status` 改为新云端结果；失败时保留原时间、原位置和失败摘要，便于继续重试。 |
| 统计分析汇总 | `test -s /mnt/sdcard/images/upload_history.json; grep -c '"upload_time"' /mnt/sdcard/images/upload_history.json` | 统计页通过 `uploadHistory` 模型读取同一份 JSON，不额外写统计数据库 | 统计页 `总记录` 与 JSON 中 `upload_time` 条目数量一致，分布概览五项分两列完整显示，最近记录表可跳转历史详情；没有历史记录时显示空状态。 |
...
| 参数与告警 UI 状态 | 屏幕查看 `参数设置`、`告警维护`；源码静态检查 `rg -n "settingsSummaryText|alarmSnapshotText|evaluateRuntimeAlarms|recordAlarmIssueToSdCard|alarmHistoryListView|alarmAdvicePanel|alarmAdviceDetailOverlay|alarmAdviceDetailFlickable" qml/Main.qml` | QML 按钮点击更新本地属性、提示条和 `alarmHistoryModel`；保存诊断调用 C++ 控制器写时间戳 `qt_alarm_snapshot_*.txt`；新问题调用 C++ 控制器写时间戳 `qt_alarm_*.log`；处理建议短面板和完整弹层共用同一组 `alarmSourceAdvice()` 排查内容。 | 参数页第一版不写真实配置、不向 F4 下发；告警页会记录真实健康/检测链路问题，但 `清故障` 不等于真实 F4 联锁解除。 |
| 告警日志/快照文件 | `ls -lt /mnt/sdcard/logs/qt_alarm_*.log /mnt/sdcard/logs/qt_alarm_snapshot_*.txt 2>/dev/null | head; file=$(ls -t /mnt/sdcard/logs/qt_alarm_snapshot_*.txt | head -n 1); test -s "$file"; tail -n 30 "$file"; log=$(ls -t /mnt/sdcard/logs/qt_alarm_*.log | head -n 1); test -s "$log"; tail -n 40 "$log"` | `saveAlarmSnapshotToSdCard()` 和 `recordAlarmIssueToSdCard()` 都先确认 `/mnt/sdcard` 挂载、创建 `/mnt/sdcard/logs`、检查可写，再写入并执行 `fsync`；可由屏幕按钮、自动告警或自检触发 | 文件非空，包含当前告警码、状态、相机/视频/存储摘要、最近告警历史和排查建议；点击后不需要固定 `sleep`，如需拔卡仍要执行 `sync` 或 `sdcard-safe-remove`。 |
...
| SSH 双模型检测 | 开发板 SSH | `cd /root/qt_camera_display && ./qt_camera_display --detect-self-test` | 输出 `RESULT status=GOOD|BAD ... fused_status=GOOD|BAD|REVIEW fused_result=good|bad|review segment_status=OK|NG ... upload_status=OK|FAIL`，并追加 `/mnt/sdcard/images/upload_history.json` | 查 `/tmp/uvc-kms-overlay-control.sock`、overlay 是否启动、两个模型运行库是否缺失、COS 账号和网络；若分类 `GOOD` 但 UNet `NG` 仍上传 `good`，优先确认 Qt 二进制是否包含 `fusedResultFromModelResults`。 |
