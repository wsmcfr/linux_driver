...
| 自动视觉居中路线 | 首页按 `开始` 且 F4 `START_CYCLE` ACK 后，QML 启动 `autoVisionTimer`，每 100ms 通过 overlay socket 发送 `LOCATE`；`uvc_kms_overlay` 读取原始 YUYV 中水平居中的 300px 宽全高搜索带，使用亮度连通域、密度范围和中心孔采样过滤黑色传送带误识别，并返回零件中心和 bbox；MP157 使用上方来料规则，把 `center_y` 作为 `VISION_POS.axis_px`、`height/2` 作为 `target_px` 下发 F4，由传送带完成上料和前后居中；传送带 SCAN 使用参数页 `scan_speed_rpm`，零件入画后的 TRACK/短步微调使用 `normal_speed_rpm`，自动流程不再固定限制到 40rpm；连续 3 帧满足 `abs(center_y-height/2)<=24px` 后发送 `BELT_STOP_CENTERED`；随后下发 `ACTUATOR_POS_MOVE actuator=2 direction=DOWN` 让上下轴下降固定步数，C++ 会先等 ACK，再等同一 `related_seq` 的 F4 `EVENT_REPORT actuator-move-done`，QML 只有拿到该完成详情后才复查 ROI；复查时 `errorY` 超出死区用传送带 `ACTUATOR_POS_MOVE actuator=0` 做前后短步微调，`errorX` 超出死区用摄像头左右轴 `ACTUATOR_POS_MOVE actuator=1` 做左右短步微调；确认 X/Y 都在 ROI 检测中心后才进入 `focus-settle` 等待约 3 秒，再复用现有 `检测` 链路运行双模型；模型检测完成后自动下发 `ACTUATOR_POS_MOVE actuator=2 direction=UP`，同样等待 F4 DONE 后再下发 `MODEL_READY/ARM_JOB_START` 通知 F4 和 ESP32S3 机械臂。 |
...
| 板端二进制功能标记 | 开发板 SSH | `strings /root/qt_camera_display/qt_camera_display \| grep -E 'runF4ActuatorPositionMoveAndWaitDone|BINARY_PROTOCOL_EVENT_ACTUATOR_MOVE_DONE|autoVisionHandleActuatorMoveDone|actuator-move-done|z-motion-down-wait|z-motion-up-wait|sendF4ActuatorStopNow|ACTUATOR_STOP_NOW|等待约3秒让摄像头对焦稳定'` | 能看到自动视觉、F4 执行器到位事件、Z 轴保护等待、对焦等待和强制 STOP marker，证明 QML/C++ 已重新编进当前板端 Qt 二进制。 | 若 marker 不存在，说明只改了源码或只拷了 QML，未重新交叉编译并替换 `/root/qt_camera_display/qt_camera_display`。 |
| 首页自动视觉居中和 Z 轴对焦等待 | 开发板屏幕 + F4 串口日志 | 点击首页 `开始`，把零件从画面上方放上传送带进入 ROI | 底部提示先出现 `自动流程ACK`，随后出现 `自动视觉：x=... y=... target=... error=... stable=...`，右侧偏差显示真实 `errorY`；F4 收到 `VISION_POS` 后误差逐步变小；连续居中后 Qt 发送 `BELT_STOP_CENTERED`，传送带停止；随后 Qt 发送 `ACTUATOR_POS_MOVE actuator=2 direction=DOWN`。F4 ACK 后还必须等张大头到位回包 `[addr FD 9F 6B]`，Qt 只有收到 `EVENT_REPORT actuator-move-done related_seq=<本次序号>` 后才提示 `F4已确认Z轴下降真实到位` 并进入 ROI 复查；ROI 不居中时先用传送带和左右轴短步微调，ROI 到中心后再显示 `等待约3秒让摄像头对焦稳定`，3 秒后才触发双模型检测；模型检测后 Qt 发送 `direction=UP`，同样只有收到回升 `actuator-move-done` 后才通知 F4/ESP32S3 机械臂抓取。目标出现后如黑色波形零件漏检，界面应显示 `目标短暂丢失` 或 `连续丢失但保持停机`，F4 不应收到新的 `VISION_LOST reason=1` 扫描命令。 | 若 F4 ACK 但没有 `actuator-move-done`，先确认张大头 Response 配置为 `Reached` 或 `Both`，再查 RX 接线、共地、电机地址、USART6/UART4 和 F4 日志；若 Qt 触发本地保护超时，禁止继续模型检测或机械臂抓取；若 Qt 一直未定位，查 `LOCATE` 和 `AUTO_LOCATE_MIN_LUMA_DELTA`；若越调越远，反转 F4 误差到方向的映射，不在 MP157 偷偷反号。 |
