...
| 手动控制路线 | 左侧 `手动控制` 页面提供传送带、检测辅助、安全状态、人工复核和命令日志；传送带按钮通过 `/dev/ttySTM2` 向 F407 USART1 真实下发 `BELTSCAN/BELTSTOP/BELTINFO`，F407 再由独立 USART6 控制张大头 Emm42；相机上下轴和前后轴暂不在当前界面显示，后续必须先由 F407 提供固定命令、状态回读和失败码后再接入。 |
| 参数设置路线 | 左侧 `参数设置` 页面已收敛为波形垫圈、平垫圈、弹性垫圈三种真实零件；`DetectSettingsController` 启动时读取 `/mnt/sdcard/config/defect_ui_config.json`，页面可真实调整模型阈值、复核阈值、ROI 大小、UNet 最小缺陷像素、overlay 透明度和 COS 自动上传开关；保存配置用 `QSaveFile` 原子写 JSON，下一次检测会把模型阈值传给 `defect-classify --bad-threshold`，把 ROI、overlay 透明度和最小缺陷像素传给 `defect-segment --roi/--alpha/--min-defect-pixels`；`保存配置` 和 `导出摘要` 都会追加 `/mnt/sdcard/logs/qt_settings_YYYYMMDD.log`，日志查看页刷新后可点开看到 `config_path`、阈值、ROI、UNet 像素阈值、overlay、上传策略和模型命令参数；关闭自动上传时仍保存 source/annotated 和本地历史，最终返回 `upload_status=SKIP`；F4 相关页面只说明边界并保留称重 `CAL <克重>\r\n`，不伪装下发运动参数。 |
...
| `20_uvc_camera/qt_camera_display/test_qt_kms_overlay_assets.sh` | 静态检查 overlay 源码、构建脚本、运行脚本、触摸按钮、SD 卡按钮、历史记录模型/页面、日志查看模型/页面/详情弹窗、历史检测信息紧凑显示、首页结果短字段、失败历史重新发送、统计分析页面、统计分布两列布局、手动控制页面、参数设置页面、参数详情弹层、未接硬件项清理、告警维护页面、处理建议完整弹层、4G 健康状态格、真实设备健康状态、健康检测非阻塞契约、每日检测历史/告警日志/诊断快照/参数日志落盘接口、早期静态首帧绘制器、启动动画覆盖层、QML 资源不压缩打包和默认启动契约。 |
...
| 打开手动控制 | 开发板屏幕点击左侧 `手动控制` | 实时视频 plane 被临时隐藏；页面显示传送带、检测辅助、安全状态、人工复核和命令日志；未进入手动模式时传送带巡航按钮置灰或提示先进入手动模式。 |
...
| 调整参数设置 | 开发板屏幕点击 `参数设置` 页中的 `+/-`、`切换`、`应用检测`、`保存配置`、`恢复默认`、`导出摘要` | 可调模型阈值、复核阈值、ROI、UNet 最小像素、overlay 透明度和 COS 自动/本地策略；零件只在波形垫圈、平垫圈、弹性垫圈之间循环；检测线程立即使用内存参数；`保存配置` 真实写入 `/mnt/sdcard/config/defect_ui_config.json` 并追加参数日志；`导出摘要` 不改 JSON，只把当前参数摘要追加到当天 `qt_settings_YYYYMMDD.log`；这些视觉参数不下发 F4。 |
...
| 2026-07-01 | 传送带接入 F407 真实 BELT 命令 | `DeviceHealthController` 新增 `sendF4BeltCommand()` 白名单入口，手动页 `巡航启动/停止/查询状态` 分别发送 `BELTSCAN/BELTSTOP/BELTINFO`；MP157 不直接拼张大头 Emm42 二进制帧，相机上下轴和前后轴在 F407 未定义固定服务前不在界面显示。 |
...
| F4 串口 `/dev/ttySTM2` | F4 控制器真实接入判断、HX711 称重标定和传送带高层控制 | Qt 后台线程按 115200 8N1 raw 模式发送 `STATUS\r\n`；周期心跳按 120 秒节流，人工刷新不节流；参数设置页称重标定弹窗发送 `CAL <克重>\r\n`；手动页传送带按钮只允许发送 `BELTSCAN/BELTSTOP/BELTINFO`，由 F407 `conveyor_motor_service.c` 转成 Emm42 USART6 控制；回复中包含 `ACK`、`OK`、`F4`、`READY` 或 `[INFO][BELT]` 时认为命令成功，包含 `ERROR` 时显示失败详情。 |
...
| 手动控制页面 | 调试级人工操作界面 | 传送带状态来自 `BELTSCAN/BELTSTOP/BELTINFO` 的 F407 串口回包和 `manualCommandLog`；手动页打开时通过 `setOverlayVisible(false)` 隐藏 KMS 视频 plane；相机上下轴、前后轴、急停、限位和联锁仍归 STM32F4，MP157 不直接发送 Emm42 帧，也不在 F407 未实现协议时显示假按钮或假状态。 |
...
| F4 传送带命令桥 | MP157 到 STM32F4 的高层命令桥 | 已在 `DeviceHealthController::sendF4BeltCommand()` 中实现传送带白名单命令；只开放 `BELTSCAN/BELTSTOP/BELTINFO`，避免 UI 发送 F407 未提供固定回包的运动命令。相机上下轴和前后轴后续应在 F407 先新增固定文本服务，再由 MP157 侧扩展白名单。 |
...
下面功能先记为后续实机联调任务。当前已经真实接入传送带 `BELTSCAN/BELTSTOP/BELTINFO`；相机上下轴和前后轴必须先在 STM32F407 工程中新增对应 ASCII 服务，再扩展 MP157 白名单，未接入前不在 Qt 界面显示按钮或占位状态。

| 优先级 | 待加功能 | 联调触发条件 | 实现要点 |
|---|---|---|---|
| 高 | 相机上下/前后轴 F4 服务 | F407 增加第二个 Emm42 串口服务，一个地址/通道负责上下移动，一个地址/通道负责前后移动时 | 建议 F407 暴露回零、移动、停止、查询等 ASCII 命令；MP157 只发高层命令，不直接拼 Emm42 帧。 |
| 高 | 相机轴限位和回零状态 | 上下/前后轴安装限位或零位传感器后 | F407 负责限位去抖、回零过程、越界拒绝和错误码；QML 只显示 F4 返回的状态，不用本地变量假定已回零。 |
| 中 | 复位到安全状态 | F4 已支持停止传送带、停止相机运动轴和清联锁状态时 | 一键下发 F4 高层安全复位命令，完成后刷新 `BELTINFO` 和后续相机轴状态查询；失败时保留错误码和禁止继续动作。 |
...
| 低 | 参数设置页 | 传送带速度、相机轴步距等运动参数需要现场可调时 | 当前参数页已真实保存视觉检测配置；运动参数仍不能由 MP157 直接写 Emm42，必须先由 F407 定义高层 ASCII 协议、限幅、互锁和状态回读。 |
...
| F4 传送带手动控制 | 开发板屏幕 | 点击左侧 `手动控制`，点击 `进入手动`，再点 `巡航启动`、`查询状态`、`停止` | 命令日志依次出现正在下发 `BELTSCAN/BELTINFO/BELTSTOP` 和 F4 回执；`BELTSCAN` 后传送带进入 F407 固件定义的低速巡航，`BELTSTOP` 后停止；`查询状态` 显示 `[INFO][BELT] desired=... speed=...` 等回包。 | 若 UI 显示 F4 命令失败，先用 `STATUS` 验证 `/dev/ttySTM2`，再确认 F407 已创建 `ConveyorMotorService_Task()` 且 USART6 到 Emm42 接线正确。 |
| F4 传送带 SSH 对照 | 开发板 SSH，Qt 串口占用时需先停止 Qt 或避免同时读写 | `/root/uartapp /dev/ttySTM2 115200 send "BELTINFO\r\n"`，需要观察回包时改用串口终端模式发送 `BELTSCAN`、`BELTINFO`、`BELTSTOP` | F407 返回 `[INFO][BELT] desired=...` 或 `[OK][BELT] Mode set to ...`；这证明 MP157 只发 ASCII 高层命令，实际 Emm42 帧由 F407 USART6 发送。 | 若无回包，检查 F407 USART1/MP157 `/dev/ttySTM2` TX/RX 交叉、共地、波特率、F407 `weight_service.c` 是否已把 BELT 命令转发到 `ConveyorMotorService_HandleCommand()`。 |
...
| 手动控制命令日志 | 屏幕查看 `手动控制` 页命令日志；源码静态检查 `rg -n "manualCommandLog|appendManualCommandLog|sendManualBeltCommand|handleManualAction" qml/Main.qml` | 传送带按钮经 `sendManualBeltCommand()` 写入 F4 串口并把回包追加到 `manualCommandLog`；检测当前帧、人工标记、急停模拟和清故障只写界面日志，不伪装成 F4 运动轴命令。 | 日志显示时间、命令、目标和结果；传送带记录包含 `BELTSCAN/BELTSTOP/BELTINFO` 和 F4 回执；当前界面不应出现未接运动轴成功记录。 |
| 参数与告警 UI 状态 | 屏幕查看 `参数设置`、`告警维护`；源码静态检查 `rg -n "settingsSummaryText|alarmSnapshotText|evaluateRuntimeAlarms|recordAlarmIssueToSdCard|alarmHistoryListView|alarmAdvicePanel|alarmAdviceDetailOverlay|alarmAdviceDetailFlickable" qml/Main.qml` | QML 按钮点击更新真实检测配置、提示条和 `alarmHistoryModel`；保存诊断调用 C++ 控制器追加当天 `qt_alarm_snapshot_YYYYMMDD.txt`；新问题调用 C++ 控制器追加当天 `qt_alarm_YYYYMMDD.log`；处理建议短面板和完整弹层共用同一组 `alarmSourceAdvice()` 排查内容。 | 参数页已把视觉检测配置写入 `/mnt/sdcard/config/defect_ui_config.json` 并影响下一次检测；F4 运动参数仍不在 Qt 参数页下发，告警页会记录真实健康/检测链路问题，但 `清故障` 不等于真实 F4 联锁解除。 |
