...
| 项目 | 内容 |
|---|---|
| 目标 | 零件停在摄像头 ROI 中央后，先让摄像头上下轴下降固定步数，再用传送带做前后/Y 微调、用摄像头左右轴做 X 微调，完成模型检测后立即让上下轴回升固定步数。 |
...
| 模块 | 已具备能力 | 本轮复用方式 |
|---|---|---|
| MP157 自动视觉 | 已能 `LOCATE`、发送 `VISION_POS`，连续居中后发送 `BELT_STOP_CENTERED`。 | 把原来居中 ACK 后 2 秒直接检测，改为先执行 Z 轴下降，再按 `errorY/errorX` 分别触发传送带前后微调和左右轴微调。 |
| MP157 参数页 | 已有三台步进电机地址、最小步长、速度、方向配置，并能下发 `STEPPER_PARAM_SET`。 | 给摄像头上下轴增加 `z_down_fixed_steps` 和 `z_up_fixed_steps`，保存 JSON 并下发给 F4。 |
| F4 摄像头电机服务 | 已支持左右轴、上下轴点动、停止和运行时配置，旧 `RequestForward...` 只作为兼容包装。 | 新增/复用相对位置模式请求，继续由摄像头电机任务独占 `USART6` 串行发帧。 |
...
| 4 | `前后/左右微调` | `errorY` 超死区时发送 `ACTUATOR_POS_MOVE axis=CONVEYOR dir=BACKWARD/FORWARD steps=minStep`；`errorX` 超死区时发送 `ACTUATOR_POS_MOVE axis=CAMERA_LATERAL dir=LEFT/RIGHT steps=minStep` | 传送带按相对位置模式做前后短步微调；左右轴按相对位置模式做横向短步微调。 | X/Y 都进入死区或达到重试上限。 |
...
| `1` | `ACT_CAMERA_LATERAL` | 摄像头左右电机，用于 Z 轴下降后的 ROI X 方向微调；旧 `ACT_CAMERA_FORWARD` 只作为兼容别名。 |
| `2` | `ACT_CAMERA_Z` | 摄像头上下电机，用于下降到检测高度和回升到原高度。 |
| `0xFF` | `ACT_ALL` | 停止命令专用，停止传送带和摄像头运动电机。 |

方向枚举：

| 方向值 | 传送带含义 | 左右轴含义 | 上下轴含义 |
|---:|---|---|---|
| `0` | 后退 | 左移 | 下降 |
| `1` | 前进 | 右移 | 上升 |
...
| 手动三轴 | 板端手动页 | 打开三轴弹窗，分别点传送带、左右、上下控制。 | 三页动作不会互相遮挡，摄像头轴命令走 F4 二进制协议。 | 查按钮 enabled 条件、F4 ACK/NACK 和摄像头电机任务队列。 |
