...
| 本地定位脚本：`22_4g_ppp/4g-location` | 为 Qt 顶部 `位置` 提供高德 IP 省份定位；脚本只访问 `https://restapi.amap.com/v3/ip`，不访问 GPS、AT 串口或 `/dev/ttyUSB*`。 |
...
| 顶部位置曾固定显示 `检测位` | 导航栏需要显示当前省份，但现场不需要 GPS；恢复 GPS/AT 串口会和 4G 管理口抢资源，也会在室内等待卫星 | 新增 `4g-location once`，只用高德 IP 定位接口判断 4G 出口 IP 所在省份；Qt 开机只调用一次，成功后显示省份，后续不周期访问高德。 |
...
| 配置高德 Web Key | `install -d -m 700 /etc/4g-location; printf '%s\n' '<你的高德Web服务Key>' > /etc/4g-location/amap-web-key; chmod 600 /etc/4g-location/amap-web-key` | Key 文件只保存在板端本地，不提交到 Git；也可用环境变量 `AMAP_WEB_KEY` 临时覆盖。 |
| 开机单次 IP 定位 | `4g-location once; cat /var/run/4g-location.state` | 成功时输出并缓存 `state=ip_ok`、`display=河南省`、`short_display=河南省`；无 Key 时为 `state=no_key`；网络或接口失败时为 `state=ip_failed`。 |
| 查看定位缓存 | `4g-location status` | 只读取 `/var/run/4g-location.state`，用于 SSH 排查；Qt 后续健康刷新不依赖该命令，也不会周期访问高德。 |
...
| 2026-07-02 | 新增高德 IP 省份定位脚本 | `4g-location once` 只访问高德 IP 定位接口，写入 `/var/run/4g-location.state`，供 Qt 开机单次定位显示省份；不使用 GPS、AT 串口或 `/dev/ttyUSB*`。 |
...
| 高德 IP 省份定位脚本语法 | Windows/虚拟机仓库根目录 | `sh -n 22_4g_ppp/4g-location` | shell 语法检查退出码为 0。 | 若失败，按行号修正脚本语法；不要用 `bash` 专有语法破坏板端 BusyBox `/bin/sh` 兼容。 |
| 高德 IP 省份定位功能 | 开发板 SSH | `AMAP_WEB_KEY='<你的高德Web服务Key>' 4g-location once; cat /var/run/4g-location.state; tail -n 40 /var/log/4g-location.log` | 成功时状态文件包含 `state=ip_ok`、`display=<省份>`、`short_display=<省份>`；`city/district/longitude/latitude/satellites` 可为空。 | 若为 `no_key`，检查 Key 文件或环境变量；若为 `ip_failed`，查 4G 联网、DNS、HTTPS、Key 配额和高德返回 `info/infocode`。 |
| 禁止恢复 GPS/AT 定位 | Windows/虚拟机仓库根目录 | `grep -E 'QGPS|QGPSLOC|/dev/ttyUSB|AT\\+QGPS|AT\\+QGPSLOC|coordsys=gps' 22_4g_ppp/4g-location || true` | 命令不应输出匹配行，说明定位脚本没有访问 GPS 或 AT 串口。 | 若出现匹配行，删除 GPS/AT 逻辑；本项目当前位置只允许高德 IP 省份定位。 |
